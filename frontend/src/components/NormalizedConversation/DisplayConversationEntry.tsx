...
import { Braces, FileText, MoreHorizontal, CheckCircle2, XCircle, Dot, Info } from 'lucide-react';
...
function DisplayConversationEntry({ entry, expansionKey }: Props) {
  ...
  const toolAction: any = isToolUse ? (entry.entry_type as any).action_type : null;
  const toolName = isToolUse ? (entry.entry_type as any).tool_name : null;
  ...
  const commandOutput: string | null = isCommand ? toolAction?.result?.output ?? null : null;
  ...
  const outputMeta = (() => {
    ...
    const lineCount = commandOutput === '' ? 0 : commandOutput.split('\n').length;
  ...
  const canExpand =
    (isCommand && !!commandOutput) || (toolAction?.action === 'tool' && (hasArgs || hasResult));
    ...
    <div className="px-4 py-1">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
                ...
                canExpand ? (
                  ...
                  >
                    <span className="flex items-center gap-1 min-w-0">
                      ...
                      >
                        {shouldRenderMarkdown(entry.entry_type) ? (
                          <MarkdownRenderer content={entry.content} className="inline" />
                        ) : (
                          entry.content
                        )}
                      </span>
                        ...
                        <>
                          ...
                          {commandOutput && (
                            <FileText
                              ...
                              title={outputMeta ? `output: ${outputMeta.lineCount} lines · ${outputMeta.sizeStr}` : 'output'}
                            />
                          )}
                        </>
                      ) : (
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {isCommand ? (
                      <>
                        {typeof commandSuccess === 'boolean' && (
                          <Dot className={
                            'h-4 w-4 ' + (commandSuccess ? 'text-green-600' : 'text-red-600')
                          } />
                        )}
                        {commandOutput && (
                          <FileText
          ...
          {entry.entry_type.type === 'tool_use' && toolExpanded && (
            (() => {
              ...
              if (at?.action === 'tool') {
                return (
                  <ToolDetails
                    ...
                    result={at.result ? { type: at.result.type, value: at.result.value } : null}
                  />
                );
              }
